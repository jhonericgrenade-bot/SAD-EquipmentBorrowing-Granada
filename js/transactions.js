...
async function saveTransaction(onChanged, getUser) { const message = byId('#transaction-message'); const equipmentId = Number(byId('#transaction-equipment').value); const borrowed = byId('#date-borrowed').value; const due = byId('#due-date').value; message.textContent = ''; if (!equipmentId) { message.textContent = 'Select available equipment.'; return; } if (due < borrowed) { message.textContent = 'Due date cannot be earlier than borrowing date.'; return; } const user = await getUser(); const { data, error } = await supabase.from('borrow_transactions').insert({ equipment_id:equipmentId, borrower_name:byId('#borrower-name').value.trim(), borrower_type:byId('#borrower-type').value, department:byId('#borrower-department').value.trim(), date_borrowed:borrowed, due_date:due, status:'Borrowed', user_id:user.id }).select().single(); if (error) { message.textContent = error.message; return; } const { error:updateError } = await supabase.from('equipment').update({ availability:'Borrowed' }).eq('id', equipmentId).eq('availability', 'Available'); if (updateError) { await supabase.from('borrow_transactions').delete().eq('id', data.id); message.textContent = updateError.message; return; } byId('#transaction-dialog').close(); await onChanged(); }
